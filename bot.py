    ...
    try:
        ...
        try:
            ...
            try:
                ...
                context = "\n\n".join(context_texts)
                logger.info(f"Retrieved {len(context_texts)} documents; context length: {len(context)} characters")
                
                if not context.strip():
                    await processing_msg.edit_text(
                        "К сожалению, я не нашёл релевантной информации в базе знаний для ответа на ваш вопрос. "
                        "Пожалуйста, переформулируйте запрос или уточните детали."
                    )
                    return
                ...
                logger.info(f"QA chain input keys: {list(chain_input.keys())}")
