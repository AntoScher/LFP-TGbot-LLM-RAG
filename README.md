...
- Intel Arc GPU с 2+ GB VRAM (опционально, для XPU)
 - (Для OpenVINO) `openvino`, `optimum-intel`, `transformers>=4.41`, `huggingface_hub>=0.23`
...
## ⚙️ Режимы и выбор бэкенда

Проект поддерживает три варианта инференса:

- CPU (PyTorch)
  - Простой для отладки и разработки
  - Меньше внешних зависимостей
  - Медленнее, чем OpenVINO на CPU

- CPU (OpenVINO)
  - Ускоренный инференс на CPU, обычно быстрее и экономичнее по памяти
  - Первый запуск дольше из-за экспорта/компиляции модели, последующие — быстрее
  - INT8-квантование доступно при установке NNCF (необязательно)

- XPU (Intel Arc + IPEX/ITREX)
  - Ускорение на видеокартах Intel Arc
  - Поддержка weight-only квантования (int4) через ITREX

Когда выбирать OpenVINO?
- Нет дискретного GPU, но нужна приемлемая скорость
- Требуется более низкое потребление памяти, чем у PyTorch CPU
- Нужен стабильный CPU-продакшн без CUDA/XPU

Переменные окружения (выбор бэкенда):
- `INFERENCE_BACKEND`: `cpu` | `openvino` | `auto`
- `DEVICE`: `cpu` | `xpu`
- `OPENVINO_DEVICE`: `CPU` | `GPU`
